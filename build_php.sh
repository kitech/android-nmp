...
CC="$ACC" LD=$ALD LDFLAGS="-lc" CFLAGS="-DANDROID -DNGX_SYS_NERR=1 -DIOV_MAX=16 $EDEFS_123 -I$TOP_DIR/compat -I$TOP_DIR/compat/libcrypt" ./configure --prefix=/sdcard/serv/stow/php-5.5.0 --disable-all --enable-fpm --enable-json --with-zlib --enable-zip --enable-pdo --with-sqlite3 --enable-sockets --enable-bcmath --enable-mbstring --enable-mbregex --target=arm-linux --host=x86_64;
